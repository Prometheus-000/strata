...
import { emitIdentity, FAVICON_PATH, HERO, HERO_PATHS, MARK_PATH } from './emit-identity'
...
test('the hero is the same projection at the one size a repository shows a stranger: both grounds, no colour a rasteriser cannot read', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'strata-hero-'))
  fs.mkdirSync(path.join(dir, '.strata'), { recursive: true })
  fs.mkdirSync(path.join(dir, 'src/theme'), { recursive: true })
  fs.copyFileSync(path.join(REPO, '.strata/decisions.jsonl'), path.join(dir, '.strata/decisions.jsonl'))
  fs.copyFileSync(path.join(REPO, 'src/theme/ledger.json'), path.join(dir, 'src/theme/ledger.json'))
  fs.mkdirSync(path.join(dir, '.strata'), { recursive: true })
  fs.copyFileSync(path.join(REPO, '.strata/config.json'), path.join(dir, '.strata/config.json'))
  const files = emitIdentity(dir)

  const decisions = readAll(REPO).length
  for (const [ground, rel] of Object.entries(HERO_PATHS)) {
    const svg = files[rel]
    assert.ok(svg, `${ground} hero was not emitted`)
    // The card a link unfurler and GitHub's own renderer rasterise: 2:1, and
    // every colour resolved. The stylesheet keeps oklch() because a browser
    // reads it; this file is read by things that are not browsers, and a
    // colour one of them cannot parse is an image that arrives blank.
    assert.match(svg, new RegExp(`viewBox="0 0 ${HERO.height * HERO.aspect} ${HERO.height}"`))
    assert.doesNotMatch(svg, /oklch\(/, `the ${ground} hero carries a colour a rasteriser may not read`)
    assert.doesNotMatch(svg, /var\(--/, `the ${ground} hero names a custom property nothing defines in it`)
    assert.doesNotMatch(svg, /<style>/, 'GitHub strips a style block, so the ground has to be in the file')
    assert.match(svg, /<rect width="1280" height="640" fill="#[0-9a-f]{6}"\/>/, 'the ground is painted')
    assert.match(svg, />Strata</)
    assert.match(svg, new RegExp(`${decisions} DECISIONS`), 'the caption is the size of the record it draws')
    assert.ok(!/NaN|Infinity|undefined/.test(svg))
    assert.ok(svg.includes('<path d="M'), 'the field is drawn')
  }
  assert.notEqual(files[HERO_PATHS.light], files[HERO_PATHS.dark], 'both grounds are the same picture on the same paper')

  // Same record, same lines: the hero is regenerated by every build, so it
  // has to be the same bytes when nothing decided anything.
  assert.deepEqual(emitIdentity(dir), files)
})
