 ...
 *
 * A harness that speaks the Model Context Protocol gets what the CLI gets
 * and nothing else: read what was decided, ask why, ask what came before,
 * assemble a packet, check, and decide. It is the same `decide()` the CLI
 * and the overlay call, with the same handlers applying the same changes to
 * the same projections, so nothing here is a second way to change anything.
 * Six tools, and no seventh that edits a file.
 *
 * Two roots, as in the CLI: the package this file sits in, and the product
 * whose record this is — `STRATA_ROOT`, else the nearest `.strata/` above
 * the working directory. For a while one path did both jobs, so pointing
 * the server at a product crashed on importing the code from it.
 *
 ...
 * says which surface wrote them, as it does for the CLI and the overlay.
 *
...
import { decide } from '@strata/substrate/decide'
import { readAll, history } from '@strata/substrate/log'
import { isHand } from '@strata/substrate/decision'
import { describe, formatDecision } from '@strata/substrate/format'
import { explain, formatExplanation, runCheck, formatCheck } from '@strata/substrate/check'
import { buildIndex, search } from '@strata/substrate/precedent'
import { loadSkills, assemblePacket, formatPacket } from '@strata/substrate/skills'
...
if (!ROOT) {
  process.stderr.write(`${NOT_A_PRODUCT}\n`)
  process.exit(1)
}
...
const MALLEABLE = config.malleable
  ? { root: process.env.STRATA_MALLEABLE ?? path.join(ROOT, config.malleable.root ?? '.'), source: process.env.MALLEABLE_ROOT ?? config.malleable.source }
  : null
  ...
  switch (method) {
    case 'initialize':
      ...
      return reply(id, {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'strata', version: '0.2.0' },
        instructions:
