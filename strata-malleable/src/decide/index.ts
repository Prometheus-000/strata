...
import { handlerFor, registerHandler, type Applied, type Handler, type Refused, type Request, type ResolvedContext } from '@strata/substrate/decide'
...
export interface MalleableHome {
  ...
  source: string
  /** Where the record lives, when it is not `root` — the product's root. */
  logRoot?: string
}
...
export function registerMalleable(home: MalleableHome): void {
  ...
  // The seeds are the theme projection's: it validates them, puts them on the
  // record and re-emits the tokens. This layer chains onto it — the identity's
  // pattern for deviations — and adds the store, so one decide() writes the
  // ledger, the stylesheet, the contract and the overrides together. Where no
  // theme projection is mounted (the library run alone), the store is all
  // there is, and it is applied here.
  const priorSeed = handlerFor('seed') as Handler<SeedRequest> | undefined
  registerHandler<SeedRequest>('seed', (req, ctx, log) => seedHandler(req, ctx, home, log, priorSeed))
  registerHandler<ShipRequest>('ship', (_req, ctx) => shipHandler(ctx, home))
  // The file's name relative to the product root, and the same computation
  // for the key `project` returns: the name used to be taken relative to the
  // working directory, so `strata import` run from two directories would
  // have imported the store twice under two `via` strings.
  const storeFile = (root: string) => path.join(path.relative(root, home.root) || '.', STORE_PATH).replace(/^\.\//, '')
  registerProjection({
    name: storeFile(home.logRoot ?? process.cwd()),
    import: () => importStore(home),
    project: (root, log) => ({ [storeFile(root)]: projectStore(log) }),
  })
  registerMalleableEvaluators(home)
  registerState('structure', () => {
    try {
      return within(home.root, () => formatStructure(buildStructure(home.source)))
    } catch (err) {
      return `(no app tree here — ${err instanceof Error ? err.message : String(err)})`
    }
  })
  registerState('drift', () => {
    try {
...
const sameSeeds = (a: ThemeSeeds, b: ThemeSeeds) =>
  (['hue', 'chroma', 'warmth', 'energy', 'density'] as const).every((k) => a[k] === b[k]) && a.appearance === b.appearance && (a.lightness ?? 0) === (b.lightness ?? 0)

function seedHandler(req: SeedRequest, ctx: ResolvedContext, home: MalleableHome, log: readonly Decision[], prior?: Handler<SeedRequest>): Applied | Refused {
  if (!req.seeds || typeof req.seeds.hue !== 'number') return { refused: 'a retheme needs the seeds — hue, chroma, warmth, energy, density, appearance' }
  const store = readStore(home.root)
  const applied = prior ? prior(req, ctx, log) : null
  if (applied && 'refused' in applied) return applied
  const body: DecisionBody = applied?.body ?? { kind: 'seed', seeds: req.seeds, from: store.seeds }
  const unchanged = applied ? !!applied.unchanged : sameSeeds(store.seeds, req.seeds)
  if (unchanged && sameSeeds(store.seeds, req.seeds)) return { ...(applied ?? {}), body, unchanged: true }
  if (!ctx.dryRun) writeStore({ ...store, seeds: req.seeds }, home.root)
  return { ...(applied ?? {}), body, written: [...(applied?.written ?? []), STORE_PATH] }
}
...
function shipHandler(ctx: ResolvedContext, home: MalleableHome): Applied | Refused {
  ...
  const notes = [...result.moved, ...result.edits.map((e) => `${e.file}: ${e.what}`), ...result.refusals.map((r) => `refused: ${r}`)]
