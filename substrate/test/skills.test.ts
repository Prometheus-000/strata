...
test('a skill parses, loads from its directory, and assembles a packet with rules, precedent, state and examples', () => {
  ...
  assert.deepEqual(p.rules.map((r) => r.id), ['voice.one-filled-action'], 'an unknown rule id is not invented')
  assert.deepEqual(p.rulesMissing, ['nope'], 'and it is named rather than silently dropped — a renamed rule empties the packet that cited it, quietly, unless someone says so')
