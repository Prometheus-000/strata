...
const d = (body: Omit<Decision, 'id' | 'at' | 'decided' | 'written' | 'via' | 'consequence'> & Partial<Decision>): Decision => {
...
const move = (region: string, from: string, to: string, line = 10, by: 'human' | 'agent' = 'human') =>
  d({ kind: 'move', region, from: { file: 'Page.tsx', line: 5, container: from }, to: { file: 'TopBar.tsx', line, container: to, index: 0 }, decided: { kind: by } })
...
test('append then readAll round-trips, oldest first, and a malformed line names itself', () => {
  ...
  assert.throws(() => append(dir, { ...a, decided: 'robot' as never }), /malformed/)
})
...
test('since finds what happened after the last handoff', () => {
  ...
  const ready = d({ kind: 'ready', decided: { kind: 'agent' } })
...
test('moves keep their order and author; an exact reversal is a change of mind, not two moves', () => {
  ...
  assert.deepEqual(collapseReversals([a, b]).map((m) => [m.kind === 'move' && m.region, m.decided.kind]), [['Filters', 'human'], ['Badge', 'agent']])
