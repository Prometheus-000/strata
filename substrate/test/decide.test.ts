...
const ctx = (root: string, extra: Partial<Parameters<typeof decide>[1]> = {}) => ({ root, decided: { kind: 'human' as const }, written: { kind: 'human' as const }, via: 'test', at: '2026-09-03T12:00:00.000Z', ...extra })
...
test('a decision is applied by its projection, then appended with provenance and a supersedes chain', () => {
  ...
  assert.ok(first.ok)
  const second = decide({ kind: 'token', token: '--x', action: 'cut' }, ctx(root, { decided: { kind: 'agent' }, written: { kind: 'agent' }, at: '2026-09-03T12:00:01.000Z' }))
  assert.ok(second.ok)
  ...
  assert.deepEqual(log[1].decided, { kind: 'agent' })
...
test('ready is built in: it records only, and counts what it hands off', () => {
  ...
  const ready = decide({ kind: 'ready' }, ctx(root, { decided: { kind: 'agent' }, written: { kind: 'agent' }, at: '2026-09-03T12:00:02.000Z' }))
